...
We will see a quick setup to configure the ``tool_env`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_env`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_env`` formulas under ``/srv/salt/tool_env/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_env/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_env/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_env/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_env/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_env`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_env`` formulas under ``/srv/salt/tool_env/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_env/parameters/roles
...
We create a configuration for the role ``tool_env/server`` in ``/srv/salt/tool_env/parameters/roles/tool_env/server.yaml``:
...
We create another configuration for the role ``tool_env/client`` in ``/srv/salt/tool_env/parameters/roles/tool_env/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_env/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_env/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_env/parameters/dns:domain/  '/srv/salt/tool_env/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-env-as-json:
      test.nop:
        - name: {{ env | json }}
...
.. code-block:: sls
    ...
    env-config-file-file-managed:
      file.managed:
        - name: {{ env.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='env-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``env`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ env['winner'] }}
